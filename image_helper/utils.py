...
def bbox_to_anchors(bbox: Sequence[float],
                    anchor: str = None) -> Union[np.ndarray, Tuple[float]]:
    ...
    if anchor_h == 'l':
        ...
        raise ValueError(f"Unknown anchor[0] '{anchor_h}'. "
                         "Value choices: 'l', 'm', 'r'")
    if anchor_v == 't':
        y = top
    elif anchor_v == 'm':
        ...
        raise ValueError(f"Unknown anchor[1] '{anchor_v}'. "
                         "Value choices: 't', 'm', 'b'")
    return x, y


def anchor_to_bbox(anchor_xy: Sequence[float], bbox_size: Sequence[float],
                   anchor: str = 'lt') -> Tuple[float]:
...
def get_bbox_rel_to_bbox(
    anchor: str, rel_bbox: Sequence[float],
